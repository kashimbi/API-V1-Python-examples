...
# Check if API responded with success
	...
	for station in decodedResponse['stations']:
		...
		while (lastMeasurement < station['lastMeasurement'][0:13] and attempt < (2 * 4 * 13)):

			attempt 				+= 1;

			# Request timeseries for specific station
			tsResponse 				= apiRequest("https://tahmoapi.mybluemix.net/v1/timeseries/" + station['id'] + "/hourly", { 'startDate': lastMeasurement })
